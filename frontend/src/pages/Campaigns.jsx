...
export default function Campaigns() {
  ...
  const cols = [{ label: "Name", key: "name" }, { label: "Channel", key: "channel", render: (r) => sentenceCase(r.channel) }, { label: "Audience", key: "audience", render: (r) => sentenceCase(r.audience) }, { label: "Sent", key: "sent_count" }, { label: "Opened", key: "opened_count" }, { label: "Replies", key: "reply_count" }, { label: "Status", key: "status", render: (r) => <Badge>{r.status}</Badge> }, { label: "Actions", key: "actions", render: (r) => <div className="row-actions"><button onClick={() => setModal(r)}>View</button>{can(user, "campaigns", "send") && <button onClick={() => send(r.id)}>Send</button>}</div> }];
