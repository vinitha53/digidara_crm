...
export default function Employees() {
  ...
  const deleteEmployee = (employee) => {
    if (!window.confirm(`Permanently delete ${employee.name} from the database? This cannot be undone. Their leads, customers and tasks will be kept but unassigned.`)) return;
    api.delete(`/employees/${employee.id}`).then(({ data }) => {
      setToast({ type: "success", message: data?.message || "Employee access removed." });
      return load();
    }).catch((error) => setToast({ type: "error", message: error.response?.data?.message || "Employee could not be deleted." }));
  };
  ...
  return <div className="page employees-page">
    ...
    {tab === "add employee" && can(user, "employees", "create") && <Card className="employee-create-card"><div className="employee-form-head"><div className="employee-form-icon"><IconUserPlus /></div><div><span>New CRM user</span><h2>Create employee access</h2><p>The employee receives a unique login ID and the permissions assigned to the selected role.</p></div></div><EmployeeForm form={form} setForm={setForm} roles={assignableRoles} showPassword={showPassword} setShowPassword={setShowPassword} showConfirm={showConfirm} setShowConfirm={setShowConfirm} onSubmit={saveEmployee} saving={saving} /></Card>}
