  ...
  return (
    <div className="table-wrap data-table-wrap">
      <table className="data-table">
        <thead><tr>{columns.map((c) => <th key={c.key}>{sentenceCase(c.label)}</th>)}</tr></thead>
        <tbody>{data.map((row) => <tr key={row.id} onClick={() => onRow?.(row)}>{columns.map((c) => <td key={c.key} data-label={sentenceCase(c.label)}>{c.render ? c.render(row) : row[c.key]}</td>)}</tr>)}</tbody>
      </table>
    </div>
  );
}
